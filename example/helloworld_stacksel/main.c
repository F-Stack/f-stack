...
/* Use FIONBIO rather than fcntl(F_SETFL, O_NONBLOCK): F-Stack keeps the
 * FreeBSD flag values, so the Linux O_NONBLOCK bit would be a no-op here.
 * ff_ioctl also mirrors FIONBIO onto the paired host fd of a dual-stack fd. */
static int
set_nonblock(int fd)
{
    int on = 1;
