...
| **PT-NR-08**（v1.2 新增，RV6；**v1.4 恢复为合入门槛，不可省略**） | **自驱 hardclock 精度回归**（C-NR-307）。**v1.4：同 lcore_id 定案后 C-NR-307 为必需项，本项是 M2 的合入门槛** | E-NR-03 采集的改造前基线（`rte_timer` 挂槽基线） | ① RTO/keepalive/延迟 ACK 实测精度与基线偏差 ≤5%（阈值待 M0 校准）；② 无漏触发/重复触发；③ `graceful_reload=0` 与改造前**行为等价**（QPS/时延在噪声区间内）。**不通过则 M2 不得合入** |
