...
| 对齐基线 | [06-方案设计](06-solution-design.md) **v1.9.5** / [07-里程碑](07-milestones.md) **v1.13**（本行自 v1.5 起未随版本同步刷新，v1.7/v1.8/v1.9/v1.10 借 M4/M5/M6/M6b 回写顺手更正为当前版本；各版的历史对齐关系以两篇的「修订记录」行为准） |
...
| PT-NR-05 | **循环 reload 门禁**（[01](01-vpp-vcl-research.md) §6.3-7 适配 v1.6 防重入语义） | — | 分档：M4 20 次 → M6 **≥100 次**；持续流量 + **每 5s 检测共享内存状态所有 G_old 已退出后 reload 一次**（reload 成功计算一次）；判据：错误数 0、无死锁无 crash、无内存泄漏趋势。VPP 前车之鉴（#3547 连环 crash、#3645 卡死，[01](01-vpp-vcl-research.md) §4.3）正是此门禁要拦的。v1.6 修订原因：v1.3 防重入语义（排空前拒绝新 reload）+ 25s 初始化时延使「每 2~5s reload、≥1000 次」不可同时成立——防重入下 999 次会被拒。**【M6 实机：PASS（终门禁关闭，A-NR-16 达成）】** **100/100 轮全部 complete**；fresh_n=7891 **fresh_fail=0**（历史最佳——M4 预演 50/50 带 10.7% fresh_fail 残留）；无死锁无 crash（每轮 drain 恒 1000~1001ms 自然完成 = watchdog 1s 节拍，零 deadline forced、零防重入拒绝）；无泄漏趋势（rtemap 100 轮 min=max=138 斜率 0.000000、运行期 hp_free 恒 1910、终态 `CLEANED rtemap=0 hp_free=2048` 全回收）。每轮 HUP 在「最近 FSM 回 T0_IDLE + worker 数回归」双通道确认后发出（tools 侧共享内存 dump 入口尚无，日志镜像为 G-A6 A-3 认可的等价实现）。门禁配置 = 无 keepalive 纯 fresh + 显式 `worker_shutdown_timeout 15s`（RV14 结论落地），实测每轮 ~6s |
