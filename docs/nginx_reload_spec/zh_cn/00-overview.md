...
- **S3 推荐理由**：S3（原生多进程演进，**v1.9 形态 M1′：常驻 slim primary + 同队列（queue_id 代际无关映射）+ ready 后同期接管 rx+tx+listen + G_new 独占硬件、G_old 软件寄生 + 双向 per-generation `drain_ring` + flow_map 软件分发表 + rx 互斥 + **两代同 lcore_id** + **同 lcore_id 资源隔离（自驱 hardclock + 代际 mempool）** + msg_ring/KNI 代际隔离 + 心跳与 rx 交还 + ARP/NDP clone**，直接承接 primary_slim #1078）是唯一同时满足结构性前提（基线 2）、连接语义对齐（基线 1）、稳态零额外开销**须由 PT-NR-09 量化后方可成立**（DR11 定案 M-A 为主 ⇒ `cache_size=0` 存在稳态损耗，**未量化前不得称「零额外开销」**；见 [06] §4.1 D5）**【2026-09-22 同步·A01-8：恢复被删除线抹掉的量化条件，判据不降低】**、有已验证 PoC 基础、与主线演进同向、可增量交付的方案（[06](06-solution-design.md) §4.2）。
...
| **跨 master 世代隔离（M5；2026-09-17 补登记·R-14）** | 常驻 primary 内 hugepage **世代目录**（落地文件 `lib/ff_reload_gendir.c`：master epoch 铸造 / rx owner 与 kni owner 字 / slot 表 + pid liveness 回收 / (epoch,gen) 单 CAS owner 仲裁）+ ring 命名掺 epoch slot（一致性由 `tools/compat/check_ring_name_mirror.sh` 校验，**当前未进 CI**）；nginx 侧 USR2 相关状态分两处：**【2026-09-22 同步·C01-6】** 通用 reload FSM（`ngx_ff_reload_fsm.h:29-36`）只定义 `T0_IDLE…T5_GOLD_QUIT/T_ERROR`，**不含 USR2 的 PENDING/HANDED**；USR2 专属状态机在 `app/nginx-1.28.0/src/os/unix/ngx_process_cycle.c`（`ngx_ff_usr2_state`，:103 定义、:370/:493-501/:527 判定，配合 `ngx_ff_usr2_begin/handover/reclaim/check` :54-58）。**另注：WINCH 注册不等于全员 READY**，不得以注册数代替就绪判定 |
...
> **【2026-09-22 同步·第四轮真机与 G-F 初步结项】** 生产形态下 precheck / rt01 / rt02（修复后）/ gr0 / rt12 / baseline / rv9-IPv4 100 轮 / rt01-IPv6 / rv9-IPv6 20 轮**均达判据**；故障形态的 rt20 / rt21 / rt22 / rt20b **仅作故障路径判据，不进功能验收**；rt13（zc）**永久排除**。终门禁 G-F **PASS（附条件）→ 初步可用、初步结项**；F2/F3 场景、跨进程 MP 集成与 G-D 英文译审列为待办（本轮不做实际动作）。详见 `work/recheck-20260921/gf-final-gate-20260922.md`。
...
| F. 测试环境局限 | 本机仅 virtio 系 PMD（**virtio stats 无 `imissed`，host 侧丢包 guest 不可观测**；物理网卡上的 rx 接管互斥 RV3 未验证）、wrk 不可用（ab + python 探测替代）、本机 IPv6 存在独立 13→15 回归问题（IPv6 reload 回归暂不覆盖）、单机客户端瓶颈、多 port 异构未决 | — | [08](08-testing.md) §1/§5/§6  **【2026-09-22 更新】本机 IPv6 已实测（rt01-IPv6 PASS、rv9-IPv6 20/20），本行「IPv6 reload 回归暂不覆盖」为历史口径** |
...
| **H. 2026-09-22 结项状态（新登记）** | **初步可用（G-F PASS 附条件）**；未执行项：F2/F3 故障与边界场景、跨进程 MP 集成、G-D 英文译审（IPv6 百轮与 rt12 管理面证据链已于 2026-09-23 闭环） | 已登记待办 | `work/recheck-20260921/gf-final-gate-20260922.md` |
