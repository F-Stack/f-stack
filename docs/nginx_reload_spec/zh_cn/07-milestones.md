...
| 版本 | v1.17（v1.16 基础上：终门禁 N1/N3 定向整改 + O-3——DR6 执行者口径统一为「G_old worker 自治夺回 rx」、心跳校准口径统一为「见 [08] §5-14（当前 UNVERIFIED）」；v1.16 = RT 结果收口；v1.15 = 同步第二轮真机回归；v1.14 = 本轮 plan_audit 交叉审核订正） |
| 日期 | 2026-09-09（v1.14~v1.17 增补：2026-09-17） |
| 状态 | 待人工审计（v1.4 人工决策落盘） |
| 对齐基线 | [06-方案设计](06-solution-design.md) **v1.9.6**（v1.6：同队列 + 同期接管 + flow_map；**v1.9.1 人工决策：D-A 反转为两代同 lcore_id ⇒ 取消四链解耦/代际 lcore 池（C-NR-311、DR5），自驱 hardclock 与代际 mempool 恢复为必需项（C-NR-307/308/314/315），新增心跳与 rx 交还（C-NR-316）**；M1′ 单硬件所有者 + G_old 软件寄生 + 双向 drain_ring 维持不变；v1.9.4/v1.9.5 为 M4/M5 实现批注版） |
...
> **锚点基线声明（2026-09-17）**：本清单各 C-NR 的「文件:行号锚点」为**编码前基线**；`lib/` 侧行号已于 2026-09-17 按 HEAD `28e751259` 复核订正（M1~M6 实现后 `lib/ff_dpdk_if.c` 由 4233 行增至 5046 行，锚点整体漂移 +300~+800），后续以 `grep -n` 实测为准。§2.5 表内 `ngx_process_cycle.c` 侧锚点同样以 `work/impl/m4-anchor-verifier.md` 基线总表为准。
...
| RT-09 kill primary | PASS | T3 窗口内 kill 数据面零中断（12/12 流）；attach gate（F-M3-2）0ms 即时 abort |
...
| 编号 | 内容 | 状态 |
|---|---|---|
| F2 | 脏 EAL respawn | **维持登记 M6**（用户裁决）；含 core_pattern 尸检 |
| F-M3-1 | G_old 退出时 RST 在途连接 | **已修**（S2 12/12 零 RST 零尾丢，跨 M3→M4 实证） |
| F-M3-2 | master 对 worker attach 失败不可恢复 | **已修**（RT-09 场景 3：0ms 即时 abort，G_old 全程零影响） |
...
1. **F2 专项**：脏 EAL respawn（含 core_pattern 尸检）。
...
| 项 | 状态 | 建议 |
|---|---|---|
| **F-M6-5（P2）** | `config.ini` 的 `[port0] addr` 在 HEAD `a8966870e "Modify config.ini."` 被写成一个本机测试用私有地址（`<LOCAL_TEST_ADDR>`，RFC1918，**已入库**；具体值见 `work/impl/m6b-coder-harness.md` §4）。任何以仓库 `config.ini` 为模板（`--fstack-conf` 默认值）的运行都会把目标地址取成它，表现为「TCP 全端口 403（igate IDC 访问页）+ ICMP TTL exceeded」，本轮两个 role 均被误导为「网关封禁/环境阻断」 | **转 leader 裁决**：forward-fix 回正确值，或按规约改回本地未提交值；**不建议**再以仓库 `config.ini` 当默认模板 |
