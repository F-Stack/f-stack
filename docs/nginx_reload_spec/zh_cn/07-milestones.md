...
| C-NR-404 | `ngx_ff_reload.c` 异常分支 | 修改 | ① T2 前 G_new 任一 worker 起不来/READY 超时 → 放弃本轮（G_old 未动，对齐内核 HUP 配置失败回滚语义）；② T3 接管失败（G_old 未确认停 rx / G_new 起 rx 超时）→ 放弃本轮（互斥标记保证不会并发 rx poll）；③ primary 被 kill → 本轮 reload 安全放弃 + 告警（primary_slim E10：primary 不可复活）；**④ 【v1.9 新增·DR6 必答项】T3 之后 G_new 崩溃**——M1′ 下 G_old 已脱离硬件、无法自行恢复 rx（这是相对 v1.6 形态**新增的风险**），须定案回退目标态：由 primary 将 rx 交还 G_old（若仍在）/ 触发新代际重生 / 整体降级为有损 reload，并给出检测手段（心跳/SIGCHLD）与超时。**【M4 实现】** ①/② 为 M2 既有结构，本轮补齐的是 **abort 全 8 调用点走查 + P0-1 修复**：hup 在 EV_HUP 接受后、任何 spawn/abort 之前前置 `ngx_memzero(ngx_ff_reload_new_slots)` —— 原实现 new_slots 唯一写点在 spawn 快照处，`init_cycle` 配置失败与 worker attach 失败两个 abort 都在快照**之前**，round≥2 时会拿到上一轮陈旧快照（即当前活跃代际）并 TERM 全部活跃 worker，与「G_old keeps serving」相反；修复后这两处只 TERM 本轮 just_spawn 集合（分别为空集与 worker 0 单点）。③ primary 被 kill 实测三态：T3 窗口内 kill → 数据面零中断、本轮自然完成；死后残留 rte 文件再 reload → 仍成功；rte 文件清除后 reload → worker attach 失败**0ms 即时 abort**（F-M3-2 修复实证，G_old 全程零影响）。**④ DR6① 维持方案①，但按 M3 已实现的形态表述为「G_old worker 自治夺回」**（执行者由 primary 改为 G_old 自身，心跳载体/1s 超时/`rx_owner_gen` 回退三点语义等价，且自治形态天然防空交还；spec 已按此修订，见 [06](06-solution-design.md) §6.2 DR6 附则）——**不得按本行字面重做 primary 回写，否则引入双写者竞态**。M4 只补收尾（D-M4-2）：夺回侧写 epoch-tagged `reclaim` 共享字 + 告警秒级限频（`ff_reload_stall_warn`，F-M3-4 修复），master 在 t3_check 轮询该字（epoch 匹配）→ EV_ABORT 回滚（TERM 残留 G_new + 关窗 + `ff_reload_master_abort` 复位 owner，与夺回方所写值幂等）；**flow_map teardown 为结构性 no-op**（G_old 从未 arm 过 dispatcher —— `ngx_ff_flow_map_arm` 仅 init 期受 `hw_locked && gen==target_gen` 双门控调用；失活 G_new 的进程局部状态随其死亡消亡）。T5 期间不检测（FSM 表 T5 仅接受 GOLD_EXITED），该窄边归 M6 |
