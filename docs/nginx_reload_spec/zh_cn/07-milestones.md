...
本文把 [06-方案设计](06-solution-design.md) 的推荐方案 S3（M1 大阶段：乒乓核 + reta 切流 + dispatch_ring 转发兜底，承接 primary_slim；M2 大阶段：可选 dispatcher 中心化）拆解为 **M0 预研 → M1 常驻 primary 化 → M2 新旧并存 → M3 原子切流 → M4 drain 收尾 → M5 USR2 → M6 门禁收尾 → M7（可选，DR7 触发）** 共 8 个里程碑、**35 个编码改动点（C-NR-100~604）**，逐项给出文件:行号锚点、依赖、DoD、测试门禁与回退方案；并把 RV1-9 逐项分配到里程碑、DR1-7 标注为对应里程碑开工前置评审。测试计划（16 条单测 UT-NR-01~16 + 5 条真 EAL 集成用例 IT-NR-A + 15 行实机用例 IT-NR-RT + 6 条性能基线 PT-NR + 20 项验收标准 A-NR + VPP 式循环 reload 门禁）见 [08-测试计划](08-testing.md)。
...
| RV9 | 端到端循环 reload 门禁（数千次零错误） | M4（50 次小规模） | **M6（终门禁，≥1000 次）** |
...
| DR3 | reta 切流 vs 队列移交 | **M0**（RV2/RV3 数据定案） |
| DR4 | ff_msg ring 作 reload 控制通道的容量/实时性 | **M2 开工前** |
| DR5 | 乒乓 lcore 配置表达与校验链 | **M2 开工前** |
...
| 项 | 内容 |
|---|---|
| **目标** | 关闭全部 RV：RV9 终门禁（循环 reload ≥1000 次，对齐 [01](01-vpp-vcl-research.md) §6.3-7 VPP 模式）、RV8（KNI）、zc 场景回归、文档入库 |
...
|---|---|
| **前置** | M4（M5 可并行推进） |
| **DoD** | ① RV9：持续流量 + 每 2~5s reload，≥1000 次零错误、无死锁无 crash、mbuf/内存无泄漏趋势；② RT-12/13 通过（或出明确结论与限制声明）；③ 全部 RV1-9 关闭记录在案；④ 文档入库 |
