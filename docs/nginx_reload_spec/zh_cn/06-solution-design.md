...
**【2026-09-17 订正·R-05】上述「例外论证」措辞不完备，不得再按原文引述**：官方原文为 *"…can cause corruption of memory pool caches, **among other issues**."* —— mempool cache 只是**举一例**，禁令的理由**包含但不限于** mempool cache 损坏。本方案仅对**已识别的两条**按 lcore_id 索引的共享槽位做了隔离（mempool 经语义 12 / DR11 处理、timer 经语义 11 处理），**其余按 lcore_id 索引的共享槽位风险未穷举**：`lib/ff_dpdk_if.c:1511-1516` 的代码注释自述在同 lcore_id 场景已踩到 **N-1 memset trigger** 与 **N-5 locked-critical-section crash** 两类**非 mempool** 故障。故正确表述为「**已识别路径已隔离、其余未穷举**」，而非「理由不成立」；须由 RV1/RV6/RV15 + PT-NR-08/09 + IT-NR-A13 实测佐证。**另（R-01）**：即便 mempool 这条已识别路径，代际应用池的跨代 free 也**未修干净** —— 见 §6.6.2 第 4 行。**补注（R-12 连带）**：官方原文以 logical cores / threads 表述，本方案的「跨进程同 lcore」属**等价外推**，非字面覆盖。
