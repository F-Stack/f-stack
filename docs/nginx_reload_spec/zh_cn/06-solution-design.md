...
11. **自驱 hardclock，绕开 DPDK 共享 timer 槽**（**v1.9 终版定案：必需项**，因 D-A 定案「同 lcore_id」而恢复；v1.7 据 2026-08-24 定时器调研+人工决策）：新旧进程**同 lcore_id** 共存时，DPDK `priv_timer[lcore_id]` 共享槽不可用（`rte_timer_meta_init` 的 memset 会踩掉 G_old 挂着的 timer 链表 → 其 hardclock 永不再触发 → 存量连接 RTO/keepalive 静默停摆；且共享链表上挂对方进程私有地址指针，`rte_timer_manage` 遍历会跨进程解引用私有虚拟地址 → 崩溃/数据损坏，不可修补）。**方案**：放弃 rte_timer 挂槽方式，main_loop 按 TSC 间隔直接调用 `ff_hardclock()`（`if (next_hardclock_tsc < cur_tsc) { ff_hardclock(); ff_update_current_ts(); next_hardclock_tsc += interval; }`）；graceful 分支下 **secondary 不再注册任何 timer**；**primary 保留 `rte_timer_subsystem_init()` 以创建 `rte_timer_mz` memzone**（`lib/ff_dpdk_if.c:1517-1518`），`rte_timer_meta_init/rte_timer_init/reset` 一并去掉，改为计算 `interval = hz_tsc / freebsd.hz`（默认 100 → 10ms）；`graceful_reload=0` 分支四连完整保留、行为逐字不变（`:1534-1542`）。每进程 hardclock 完全独立、零共享状态，同 lcore_id 无冲突。依据：F-Stack 全树 `freebsd_clock` 是唯一 rte_timer 用户（lib/app 仅 ff_dpdk_if.c 两处），不依赖 rte_timer 任何跨核/同步特性。改动点：`ff_dpdk_if.c` init_clock/init_clock_worker/main_loop 三处。对 `graceful_reload=0` 等价生效（自驱与挂槽行为等价），需回归 timer 精度（RTO/keepalive）。
   **v1.9 终版定案（D-A = 同 lcore_id）**：`priv_timer` 必然同槽，故本项**恢复为 M1′ 必需项**，不再是可选项。附加收益有二——(a) 无需依赖 `--lcores` 映射的正确性；(b) M1′ 下 G_old 不再 poll 硬件、main_loop 语义变「纯软件循环」，自驱 hardclock 免去了对 `rte_timer_manage` lcore 语义的依赖，实现更简单。**回归要求**：PT-NR-08 精度回归为**合入门槛，不可省略**【2026-09-22 同步·M03-1：判据为功能口径**单次偏差 ≤1 tick**；原「精度与 `graceful_reload=0` 基线偏差 ≤5%（阈值 M0 校准）」所依赖的挂槽基线不可测（P0-NEW），已取代】；RTO/keepalive/延迟 ACK **单次偏差 ≤1 tick**（PT-NR-08 功能口径；短定时器按绝对 ≤1 tick 与相对 ≤5% 取严者，分母取配置值），且无漏触发/重复触发。
...
| zc_stack（零拷贝） | **正交**：零拷贝收包路径不影响队列所有权模型；回调判定发生在包分发层，与 mbuf 来源无关（**2026-09-22 决定：zc 形态永久排除，本仓库不构建、不测、不进行支持；原「需在 M1 测试中纳入 zc 场景回归」要求已作废**） |
...
| RV6 | **自驱 hardclock 精度等价性实测**（**v1.9.1 改写**：D-A 定案「两代同 lcore_id」⇒ `priv_timer` **必然同槽**，C-NR-307 是唯一解法、**无退路**；本项按 PT-NR-08 功能口径验证自驱模式精度（**单次偏差 ≤1 tick**）【2026-09-22 同步·M03-1：`rte_timer` 挂槽基线不可测（P0-NEW 坐实 timer 从未挂载），原「精度等价性」表述作废】，而非验证"隔离") | 【2026-09-22 同步·M03-1：挂槽基线不可测（P0-NEW），不再先采集挂槽基线】直接采集自驱数据（E-NR-03，两实例须用**相同** lcore_id）；双代际共存期间观测 RTO/keepalive/延迟 ACK 精度与漏/重复触发。**不满足 PT-NR-08 功能口径（单次偏差 ≤1 tick）即为 blocker**（PT-NR-08 为 M2 合入门槛） |
...
| 2 存量连接完整 drain | G_old 活着跑 ff_run（无硬件模式）+ flow_map miss → `drain_ring_rx` 送达 + 出包经 `drain_ring_tx` 代发 + ARP/NDP clone + **timer 持续（同 lcore_id ⇒ 由自驱 hardclock 保证，语义 11 必需项）** | RV4（ring 容量与满环行为）、RV6（自驱精度按 PT-NR-08 功能口径 ≤1 tick，PT-NR-08 为门槛）、RV11（TX 独占回归） |
...
| **同 lcore_id 的 timer 同槽（语义 11，必需）** | 自驱 hardclock：main_loop 按 TSC 间隔直调 `ff_hardclock()`，每进程 timer 完全独立 | PT-NR-08 精度回归为**合入门槛**【2026-09-22 同步·M03-1：**单次偏差 ≤1 tick**（PT-NR-08 功能口径；原「偏差 ≤5%」依赖的挂槽基线不可测，已被取代），无漏/重复触发】；RV6 长稳 |
...
| ~~加固项默认态~~ | ~~自驱 hardclock、`cache_size=0`、mempool 分代际默认全部关闭~~ **已废止** | D-A 反转后前两条为必需项；`cache_size=0` 不再是全局默认，仅作为 DR11 的 M-A 选项；**【2026-09-22 同步·B01-7】** 作用域已不限于共享 RX 池——R-01 后 `graceful_reload=1` 时代际应用池亦为 `cache_size=0`（随 `graceful_reload` 开关生效，非无条件全局） |
