...
> 状态：已完成 | 生成时间：2026-07-22 | 物理机测试：2026-07-23 | 依据 spec：11/12/13
...
> 注：下表行号为规划阶段值；实现后因新增代码，部分行号偏移（如 IFCAP_LRO `:945`→`:984`、ff_veth_input `:1706`→`:1717`），以最新代码为准。
...
- **实现**：commit `8e320eeee`（软件 LRO 主体）+ `e6d64d266`（NULL timeout 修复），门禁审核通过。
- **物理机测试**：2026-07-23 完成，暂时无问题。软件 LRO 回退路径（`sw_lro=1`）IPv4/IPv6 正常、1261 规避有效、`lro=0` 零回归。
- **遗留**：物理机网卡同样不支持硬件 LRO（virtio 类），硬件 LRO 端到端聚合收包路径（`rx_lro=1` 分支 / `IT-SWLRO-13` 硬件分支 / `HB-1`）至今无运行时证据，需更换支持 `RTE_ETH_RX_OFFLOAD_TCP_LRO` 的物理 NIC 方可验证。详见 `00` 最终状态、`02` §6.5、`07` §8.2。
