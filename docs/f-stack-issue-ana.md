...
- **#520** 🟢open Disable TX ip checksum offload in VMware ESXi 5.5.0 Update 2
  - Conclusion: [Based on the latest reply, 2026-07-24] Official final confirmation: the current temporary workaround is to set `tx_csum_offoad_skip=1` in the [dpdk] section of config.ini, which disables all TX checksum offload (IP+TCP/UDP), resolving the issue where NICs in virtualized environments (such as VMware) claim to support TX checksum offload but do not actually compute the checksum. The finer-grained control suggested by the user (disabling only IP-layer check...
  - Fix/Workaround: Temporary workaround: set `tx_csum_offoad_skip=1` in config.ini (this disables IP-layer and TCP/UDP-layer checksum offload simultaneously). A finer-grained option to disable only IP-layer checksum offload has not yet been implemented (community contributions can reference the hw_features.tx_csum_ip and tx_csum_l4 fields in lib/ff_dpdk_if.c); the issue remains open. See #317 for the related root cause.
...
- **#331** 🟢open kqueue timer not usable
  - Conclusion: [Based on the latest reply, 2026-03-23; issue remains open] Official analysis: timer precision is related to the hz parameter in config.ini (default 100, i.e., 10ms per tick); increasing the hz value (e.g., to 1000 for ~1ms precision) can improve precision but increases CPU usage. The official team initially mentioned a similar issue was reported in #701 but not fully merged/fixed; subsequently, the official team updated that #701 and #702 were supposed to fix this issue, but the corresponding fix commits are missing in the current branch, requiring further investigation; iss...
  - Fix/Workaround: Temporary workaround: increase the `hz` parameter in config.ini (e.g., hz=1000) to improve precision, at the cost of increased CPU usage; the relevant fix is referenced in #701/#702 but is currently missing from the branch, pending further official investigation and implementation.
...
- **#535** 🟢open Crash on multiple IPFW rules
  ...
  - Fix/Workaround: Root cause: `MAX_MSG_BUF_SIZE` in `lib/ff_msg.h` is only 10KB; when the rule volume is large, `getsockopt(IP_FW3)` exceeds the limit and returns EINVAL at `tools/ipfw/compat.c:60`. Fix direction: increase `MAX_MSG_BUF_SIZE` or switch to dynamic buffer allocation; not yet fixed, issue remains open.
...
- **#481** 🟢open Padding bytes not removed from the ethernet frame
  - Conclusion: [Based on the latest reply, 2026-07-17] Official final confirmation: this is a genuine bug. DPDK PMD drivers do not strip Ethernet padding bytes from frames smaller than 64 bytes. F-Stack's `ff_mbuf_gethdr()` directly uses DPDK's `pkt_len`/`data_len` to set `m->m_pkthdr.len` and `m->m_len`, which includes the padding bytes. Although FreeBSD's `ip_input` has padding trimming logic (`m->m_pkthdr.len >`…
  - Fix/Workaround: Not yet fixed: the mbuf length needs to be corrected in `ff_mbuf_gethdr()` or `ff_veth_input()` based on the IP header's `ip_len` field to strip the Ethernet padding bytes. As of this analysis, the issue remains open with no fix commit observed.
...
- **#631** 🟢open ff_shutdown() not working on UDP sockets
  - Conclusion: [Reply on 2026-07-30] Official final reply: in FreeBSD, `udp_shutdown()` calls `sorflush()` to set the `SS_CANTRCVMORE` flag, which should wake up threads blocked in `recv()`; returning `ENOTCONN` for an unconnected UDP socket is POSIX-compliant behavior. F-Stack is a userspace port of the FreeBSD stack, and the `wakeup()` mechanism used to release `sbwait()` blocking may behave differently in userspace; the maintainers will investigate further; if…
  - Fix/Workaround: Root cause still under investigation: in FreeBSD userspace, the `wakeup()` mechanism used to release `sbwait()` blocking may behave differently from kernel space, causing `ff_shutdown`'s wakeup of blocked `recv()` on UDP sockets to fail. The issue remains open, pending precise reproduction steps from the user.
...
- **#701** ⚪closed F-stack kernel failing to track time properly
  - Conclusion: The user self-diagnosed and provided a temporary hack: F-Stack adapts the BSD callout to accept a "ticks" parameter rather than `bintime`; the user modified `kern_event.c` so that all values are absolute (rather than relative to `sbinuptime()`) to match this assumption, initially resolving the trigger interval discrepancy, and suggested the maintainers build a proper fix on this basis. The maintainer said they would check, but no official fix confirmation followed.
  - Fix/Workaround: Temporary workaround: modify the timer calculation logic in `kern_event.c` to use absolute values rather than values relative to `sbinuptime()`, matching F-Stack's callout assumption about the "ticks" parameter. No official fix has been confirmed. Related: #702.
- **#702** ⚪closed F-stack rack and BBR both causes PCB memory leak
  - Conclusion: [Reply on 2023-01-05, based on the latest reply] Final maintainer confirmation: bbr and rack now correctly call `hpts_timeout_dir()` with the timer and release it to the uma zone (`pcbinfo->ipi_zone`). However, HPTS's timer still has other issues to be addressed later (see #701), and the current uma zone does not return memory to the OS after release, which will also be addressed later. In 2025, another user confirmed this fix was effective.
  - Fix/Workaround: Partially fixed: bbr/rack now correctly call `hpts_timeout_dir()` to release PCBs to the uma zone, avoiding the main memory leak. Remaining issues: HPTS timer-related problems (see #701), and uma zone memory not being returned to the OS after release, both pending further adjustment.
...
- **#1063** 🟢open adapter/syscall
  - Conclusion: Official conclusion: there is currently no dedicated UDP example under `adapter/syscall`; existing examples (`main_stack_epoll.c`/`main_stack_epoll_thread_socket.c`) focus on TCP/epoll mode. UDP usage via `adapter/syscall` follows standard POSIX semantics — `socket(AF_INET, SOCK_DGRAM, 0)` + `bind()` + `sendto()`/`recvfrom()` work as expected,……
  - Fix/Workaround: UDP currently works transparently in LD_PRELOAD mode via standard POSIX semantics (socket/bind/sendto/recvfrom), with no dedicated adaptation needed. A dedicated example is planned for a future release (issue remains open).
...
- **#795** 🟢open Specifying devargs parameter?
  - Conclusion: Official conclusion: config.ini supports a subset of DPDK EAL parameters (e.g. `allow` allowlist, `file_prefix`, `nb_vdev` for virtual devices), which are translated into EAL arguments in `lib/ff_config.c`. Passing arbitrary `--device` devargs directly is not yet supported; this is planned for a future release (see #265). To use custom devargs now, modify `lib/ff_config.c` to add a configuration option and append it to `dpdk_argv[]`.
  - Fix/Workaround: Arbitrary devargs are not currently supported; requires manually modifying `lib/ff_config.c`. Related: #265 (planned feature).
