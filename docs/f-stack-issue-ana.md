...
- Total issues: 778 (open 9 / closed 769)
...
- **#331** 🟢open kqueue timer not usable
  - Conclusion: [2026-08-07 local test + fix] Confirmed the bug is still unfixed. Root cause is a two-part bug: (1) kern_event.c kqtimer_sched_callout passes absolute sbintime to F-Stack's callout_reset_sbt_on macro, which ignores C_ABSOLUTE, converts absolute sbt to absolute ticks, and callout_cc_add treats it as relative (c->c_time = ticks + absolute_ticks) → double-counting, delay grows with system uptime; (2) ff_kern_timeout.c callout_tick's softclock(cc) is inside #ifndef FSTACK (dead code under -DFSTACK), so the regular callout wheel is never driven and EVFILT_TIMER callouts never fire. Maintainer claimed "#701/#702 should have fixed this but commits are missing" — the commits (e592cbbfe/a816e8963) are actually in HEAD but neither fixes timer precision code (the former only changes config.ini hz recommendation, the latter fixes PCB leak). TCP timers ARE affected — the earlier claim that they were unaffected was WRONG (corrected 2026-09-02, binary-level proof: objdump shows callout_when compiles to a single `ret`). The empty stub never writes *sbt_out, so tp->t_timers[] stays SBT_MAX (tcp_subr.c:2304), tcp_timer_next() always returns TT_N, and tcp_timer_activate() always takes the callout_stop() branch — i.e. RTO/retransmit, keepalive, 2MSL, persist and delack were NEVER armed on the current mainline. Runtime evidence: TIME_WAIT never reclaimed within 75s (msl=2000 implies 2MSL=4s) while ff_clock advances normally at ~102Hz. The HPTS claim was also wrong: config.ini sets functions_default=freebsd, so the default stack uses t_callout, not HPTS. Syncache SYN|ACK retransmission uses a separate tick-based path, which is why connection setup still works and the defect went unnoticed. Fix planned as part of the nginx graceful reload work (implement callout_when() AND fix the callout_reset_sbt_on macro in callout.h:96-97 which treats absolute sbt as relative ticks and drops pr/C_ABSOLUTE — the two must be fixed together). Evidence: docs/nginx_reload_spec/work/impl/m0-poc-runner.md section 4.4 and m0-poc-reviewer.md section 4.B.
...
- **#631** 🟢open ff_shutdown() not working on UDP sockets
  ...
  - [2026-08-27 code analysis] Confirmed the issue DOES exist on the 1.21 branch (FreeBSD 11.0): on FreeBSD 11, soshutdown() (kern/uipc_socket.c:2349-2351) returns ENOTCONN for unconnected sockets **before** reaching sorflush(), so the receive side is never shut down for unconnected UDP sockets; p_osrel=1100122 (param.h:61 __FreeBSD_version, ff_init_main.c:370) >= P_OSREL_SHUTDOWN_ENOTCONN=1100077, so sys_shutdown()'s ENOTCONN-to-0 compat layer (uipc_syscalls.c:1340-1342) does not apply — ff_shutdown() returns -1 with ENOTCONN and ff_recvfrom keeps receiving. Connected UDP sockets are unaffected (the sorflush path works). FreeBSD 15 rewrote this path — soshutdown() now delegates to the protocol layer with the how parameter (release/2.0 uipc_socket.c:3674-3683) and udp_shutdown() handles the unconnected case (udp_usrreq.c:1742+) — which is why 1.26 is not reproducible.
  - Fix/Workaround: No fix on the 1.21 branch; users needing ff_shutdown(SHUT_RD) on unconnected UDP sockets should use the latest version (English comment posted on GitHub 2026-08-27). Detailed analysis: docs/issue_631/zh_cn/.
...
- **#1076** 🟢open F-Stack behavior at high CPS
  ...
  - [2026-08-10 Deep Research] Code-level verification confirms 4 FreeBSD native CC limit mechanisms are fully functional in f-stack: maxsockets (UMA zone max enforced, uipc_socket.c:320/uma_core.c:4461), ipfw limit (O_LIMIT→IP_FW_DENY drop, ip_fw2.c:2937/ip_fw_dynamic.c:1873), somaxconn (backlog truncation + 3*qlimit/2 check), syncache (bucket full drop oldest + pause). Only missing: mbuf pool water-level backpressure (no rte_mempool_avail check in RX path). ff_ipc uses independent message_pool not directly depending on pktmbuf_pool, but main loop cannot reach process_msg_ring when mbuf exhausted, indirectly causing ff_ipc unresponsive. Recommended Plan C (combined): lower maxsockets to match mbuf pool capacity + ipfw limit per-source restriction + optional mbuf water-level backpressure. Live testing not completed due to L2 network disconnect between DPDK NIC and client; code-level conclusions are reliable. See docs/issue_1076/zh_cn/.
  - [2026-08-10 Implementation] Plan B (mbuf water-level backpressure) implemented: added rte_mempool_avail_count check at process_packets() entry, drops TCP SYN packets when mbuf pool avail count falls below mbuf_low_watermark threshold (no SYN-ACK sent), protecting existing connections. New is_tcp_syn() helper parses Ethernet/VLAN/IPv4/IPv6/TCP headers. Config option mbuf_low_watermark (default 0=disabled, zero regression). Clean build passed, helloworld starts normally. See docs/issue_1076/zh_cn/07-mbuf背压实现报告.md.
...
- **#1078** ⚪closed primary stability control
  ...
  - Dedicated research (2026-08): verdict is **conditionally feasible and recommended**, which differs from the "substantial architectural changes" judgement above. A PoC needed only **3 lines of code plus one config item** (relax the `nb_rx_queue == 0` `rte_exit` at `lib/ff_dpdk_if.c:508-510`, fix the mbuf pool formula at `:535` so it no longer depends on the current process's queue count, and drop the primary's lcore from `[port0] lcore_list`). After killing the slimmed primary, all 12/12 established connections stayed up with zero interruption, 12/12 new connections succeeded, and there was no performance regression. Root cause: queue count and queueid are derived from the port's `lcore_list` length and index (`:836`, `:487-491`), fully decoupled from `proc_id`, so removing the primary makes queue count / queueid / RSS reta shrink consistently and orphan queues disappear naturally. Boundaries: the primary must stay resident and must not exit (DPDK keeps the IPC server, heap-growth proxy and all interrupts in the primary only); slimming does not save CPU (it still spins on a full core); the degraded control plane requires a scheduled full-group restart; KNI scenarios and non-igb_uio drivers remain unverified. Full research: `docs/issue_1078/zh_cn/` (00-10, 11 documents). Note: the "official conclusion" wording above is questionable — according to the issue page captured at the start of this research, the issue is Open with no visible comments and no associated PR (not re-verified online during this round), so the "official conclusion" is likely an inference made during earlier note-taking rather than a maintainer statement.
  - Second-round KNI deep-dive (2026-08-10): the user noted that the first round treated KNI as a marginal topic with three over-engineering issues. Three concerns addressed: Q1 `kni_tx_ring` single-consumer suffices (`kni_rp` already uses `RING_F_SC_DEQ`, keep as-is); Q2 ratelimit needs no change (`kernel_packets_ratelimit` in `kni_process_tx` is only executed by the owner, naturally single-point rate limiting); Q3 secondary can directly handle KNI (K4 scheme). E4a decisive experiment: secondary successfully discovered and probed the virtio_user port created by primary via `VDEV_SCAN_REQ` (`VDEV_BUS: receive vdev, virtio_user0`). E4b K4 PoC (7 code changes) compiled successfully, both processes ran stably, veth0 interface present. **The K4 scheme (secondary as runtime KNI owner) has been verified feasible at both code and runtime levels**, with K3-corrected (K3 + Q1/Q2 corrections) demoted to fallback. KNI boundary L3 is now closed. See `docs/issue_1078/zh_cn/11-KNI深化调研与实验报告.md` for details.
  - Implementation completed (2026-08-10): the `primary_slim` feature has been fully implemented and the issue has been closed on GitHub. An explicit `primary_slim` switch (`[dpdk] primary_slim=0/1`, default 0 = zero regression) was added; when enabled, the primary does not allocate rx/tx queues and only handles control-plane duties (NIC init, KNI init, IPC server, heap-growth proxy). Two commits (`1c28aaa2d` M1 + `f7961b083` M2-M4), 6 files, +178/-32 lines. Test results: `primary_slim=0` zero regression; `primary_slim=1` primary CPU 0.8% (with `primary_slim_idle_sleep` default 1000us), 12/12 established connections zero interruption after killing slim primary (26 rounds), 12/12 new connections succeeded, QPS no regression, 0 failed requests; KNI K4 (secondary as runtime KNI owner) verified at code + runtime level. The two original-issue judgments ("all connections affected" and "entire process group must restart") were corrected by test data. The issue was closed on GitHub with a detailed English reply summarizing the implementation, test results, DPDK constraints, and boundaries. See `docs/issue_1078/zh_cn/` for full documentation.
  - Post-implementation fixes (2026-08-11): two regressions found and fixed during testing. (1) `e075e534f` fixed KNI breakage when `primary_slim=0` + `owner_proc_id` set (doc 12); (2) `f23f1a464` + `f250ad1ea` fixed cross-process TX queue race when `primary_slim=1` + KNI enabled — introduced `kni_inject_rp` shared ring, primary's `kni_process_rx` redirects to ring, owner secondary dequeues and tx_bursts with its own `tx_queue_id`, eliminating cross-process TX queue0 desc ring corruption risk (doc 13). `da99a766f` fixed `ff_kni_enqueue` stat consistency. `180b22c21` added exit cleanup behavior analysis (doc 14).
  - **Closure (2026-08-11)**: physical machine functional and performance tests passed; default standard multi-process mode and multi-thread mode both passed 10+ minute high-traffic regression stress tests with zero regression. Boundaries L1 (PMD/driver limitation) / L2 (single port ≤3 procs) / L4 (PoC ≠ production) closed; L5 (graceful exit) / L6 (long-term stability) partially closed; L7 (performance absolute values not extrapolable) / L8 (upstream community research incomplete) remain. Full closure report: `docs/issue_1078/zh_cn/15-结项报告.md`.
...
- **#1063** 🟢open adapter/syscall
  ...
  - Fix/Workaround: [Implemented locally] Added `adapter/syscall/main_stack_udp.c` — a UDP echo server example modeled after `main_stack_epoll.c`, using `socket(SOCK_DGRAM)` + `bind(:9000)` + `epoll` + `recvfrom`/`sendto` in LD_PRELOAD mode. Makefile `example` target updated with build rule. Compiles cleanly (`make clean && make all`, `-Wall -Werror`); kernel-stack echo test passed (`127.0.0.1:9000`); LD_PRELOAD end-to-end test passed via the DPDK-owned NIC (3/3 UDP packets echoed correctly from f-stack-client to `<DPDK NIC IP>:9000`). See `docs/issue_1063/zh_cn/`. Upstream issue remains open.
