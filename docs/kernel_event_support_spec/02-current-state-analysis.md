...
> - **D13 ioctl**: kernel fd uses the **raw Linux request** passed straight to `ff_host_ioctl` (NOT via `linux2freebsd_ioctl`; `_IO/_IOR/_IOW(type,nr,size)` encodings differ between Linux and FreeBSD — cross-validated). Branch placed after `va_arg` for `argp`, before translation. **Dual-stack fd same-driver NOT implemented** (only encode kernel fd routed, conservative minimum; can later mimic `ff_fcntl` if needed).
