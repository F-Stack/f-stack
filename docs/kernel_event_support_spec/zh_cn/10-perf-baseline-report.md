...
> **文档编号**：SPEC-KE-10
> **版本**：v5（编译宏门控范式；沿用 v4 真共存口径，作废 v3 纯内核 loopback 口径）
> **日期**：2026-06-17
> **状态**：FINAL（R4 性能门禁实测产出）；**v5 注**：本基线在共存代码**已编译进** `libfstack.a`（等价 `FF_KERNEL_COEXIST` 开）、仅切运行期 `kernel_coexist` 0/1 的口径下测得，仍有效；R6 编译宏关闭态（共存代码不编译）的零回归由 `07 §1bis` MT-1 nm 符号比对验证，**无需重测性能**（宏关时与原 F-Stack 同二进制）。
> **作用域**：用实测验证「F-Stack 用户态栈 + 本地内核栈共存」对 **F-Stack 业务快路径无回归**（PERF-1/PERF-2），并给出**内核侧旁路吞吐**（PERF-3）管理面数据点。
