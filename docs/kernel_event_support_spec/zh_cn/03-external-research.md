...
> **日期**：2026-06-15
> **状态**：编写中
> **作用域**：调研其他 DPDK/用户态协议栈程序如何用"**单一 API + 标记/配置选栈**"让应用透明地在用户态栈与内核栈间分流，以及"应用作客户端连本机/外部内核服务"的处理；提炼可借鉴点与局限。所有条目附**可访问 URL**。
> **范式提示（v3）**：本特性**不**新造 `ff_local_*` 双 API，而是复用并标准化 F-Stack 现有"**单 API + `SOCK_KERNEL`/`SOCK_FSTACK` 标记 + 胶水自动适配**"，加 config.ini 一个**全局默认栈开关**；**不**做线程级选栈（多进程模型靠不同 config 文件区分）。KNI/报文回灌仅作边界澄清。
