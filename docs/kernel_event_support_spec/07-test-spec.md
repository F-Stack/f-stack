...
> - **Real-machine dual-stack (Q4=B)**: DPDK-exclusive NIC (≠eth1); one default `listen(80)` demo. IT-1 `ff_netstat`+`ss` each see 80; IT-2 remote `curl <DPDK_NIC_IP>:80` (F-Stack side, via DPDK NIC); IT-3 local `curl 127.0.0.1:80` (kernel side); IT-5 unified events both stacks. config local test values not committed.
...
> - **Real-machine (R9)**: IT-11 kqueue-model helloworld (coexist=1) kernel `curl 127.0.0.1:80=200 size=438` (was 000) + F-Stack `<DPDK_NIC_IP>:80=200` no regression; IT-12 `-DINET6` helloworld (coexist=1) starts successfully (v4+v6 listen, no errno=98), packet capture confirms kernel-side 200.
...
> - **Real-machine (R10)**: IT-13 kernel-fd readv/writev/ioctl correct on managed kernel fd (127.0.0.1), kernel `curl 127.0.0.1:80=200`; IT-14 F-Stack side `<DPDK_NIC_IP>:80=200` no regression.
