...
> - **ioctl (D13)**: kernel fd uses the **raw Linux request** straight to `ff_host_ioctl` (NOT via `linux2freebsd_ioctl`; encodings differ Linux vs FreeBSD, `03` cross-validated); branch after `va_arg` for `argp`, before translation. **Dual-stack fd same-driver NOT implemented** (only encode kernel fd routed, conservative; can later mimic `ff_fcntl`).
