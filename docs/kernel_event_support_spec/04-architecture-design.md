...
> - **Acceptance**: kqueue-model helloworld (coexist=1) kernel-side `curl 127.0.0.1:80=200 size=438` + F-Stack side `<DPDK_NIC_IP>:80=200` no regression; `-DINET6` helloworld (coexist=1) starts successfully (v4+v6 listen). R9 to-be-implemented.
>
