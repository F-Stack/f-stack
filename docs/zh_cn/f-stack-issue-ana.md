...
- **#331** 🟢open kqueue timer not usable
  - 结论：【2026-08-07 本地实测+修复】确认 bug 至今未修，根因为双重 bug：(1)kern_event.c kqtimer_sched_callout 传绝对 sbintime 给 F-Stack 宏 callout_reset_sbt_on，该宏忽略 C_ABSOLUTE，把绝对 sbt 转绝对 ticks，callout_cc_add 又当相对 ticks（c->c_time=ticks+绝对ticks）→ 双重计算，延迟随系统 uptime 增长；(2)ff_kern_timeout.c callout_tick 中 softclock(cc) 在 #ifndef FSTACK 内为死代码，普通 callout 轮从不被驱动，EVFILT_TIMER callout 永不触发。维护者称"#701/#702 应已修但 commit 缺失"——实际 commit(e592cbbfe/a816e8963)都在 HEAD 中但都不是 timer 精度代码修复（前者只改 config.ini hz 推荐，后者修 PCB 泄漏）。TCP 定时器**受影响**——此前「不受影响」的结论是**错误的**（2026-09-02 更正，二进制级证据：objdump 显示 callout_when 编译产物仅一条 `ret`）。空 stub 从未写入 *sbt_out，故 tp->t_timers[] 恒为 SBT_MAX（tcp_subr.c:2304），tcp_timer_next() 恒返回 TT_N，tcp_timer_activate() 恒走 callout_stop() 分支——即当前主线上 RTO/重传、keepalive、2MSL、persist、delack **从未被挂载**。运行时佐证：TIME_WAIT 75 秒零回收（msl=2000 ⇒ 2MSL 应为 4s），而 ff_clock 正常以 ~102Hz 推进。HPTS 的说法同样错误：config.ini 的 functions_default=freebsd，默认栈走 t_callout 而非 HPTS。syncache 的 SYN|ACK 重传走另一条 tick 制路径，这正是建连依然稳定、缺陷长期未被察觉的原因。修复计划：随 nginx 无损 reload 工作一并落地（实现 callout_when() **并**修 callout.h:96-97 宏——该宏把绝对 sbt 当相对 ticks 且丢弃 pr/C_ABSOLUTE，两处必须成套改）。证据：docs/nginx_reload_spec/work/impl/m0-poc-runner.md §4.4 与 m0-poc-reviewer.md §4.B。
