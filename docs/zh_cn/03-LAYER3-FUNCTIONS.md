...
- **R10 残余入口共存（`ff_syscall_wrapper.c`）**：`ff_readv`（L1189）/`ff_writev`（L1251）内核 fd 经 `ff_host_readv/writev`（仿 read/write，连接 fd 单栈热路径）；`ff_ioctl`（L1067）内核 fd 用**原始 Linux request** 直传 `ff_host_ioctl`（不经 `linux2freebsd_ioctl`；双栈 fd 同驱动暂未实现，仅 encode 内核 fd 路由）；`ff_dup`（L2130）内核 fd→`ff_host_dup`+encode；`ff_dup2`（L2156）两端内核 fd→`ff_host_dup2`+encode、混栈拒绝 `errno=EINVAL`。已知限制：`ff_select`（encode 内核 fd 超 `FD_SETSIZE` 硬限制）、`ff_poll`（保守未实现）不支持内核 fd 共存，改用 `ff_epoll_*`/`ff_kqueue`。
