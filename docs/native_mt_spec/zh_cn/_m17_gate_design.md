...
# 11. 收尾闭环核验（gate-design 结案）

核验对象：入库版 spec = `git show 06396b501:docs/native_mt_spec/zh_cn/17-SMP-aware-pcpu视图与去全局锁.md`（**166,343 字节**）
方式：只读核验（未做任何测试或修改），逐条确认第 10 节的 6 条必改是否闭环。

## 11.1 六条必改全部闭环 ✅

| 必改 | 状态 | 入库版依据 |
|---|---|---|
| **必改-1（F1，曾三轮未修）** | ✅ **已闭环** | `:352` **整段替换**并显式标注「原文误称『`rp_ent[]` 的每个索引都独立对 `mp_ncpus`/`rp_num_hptss` 取模』，**该表述已被实测证伪**」；`:356-360` 改用**写侧有界**论证，穷举 `t_hpts_cpu` 全树 **3** 个写入点，并给出 `hpts_random_cpu()`（`:466-475`，`:473` 双重取模）的坐实—— 与我第 2 轮给出的替换措辞一致（我核过的 `:467-474` 与其`:466-475` 在容差内） |
| **必改-2（§7 未回填 M5 DoD-5）** | ✅ **已闭环** | 入库版 `grep "0.50%\|1.67%"` **命中 3 次**（原版本零命中） |
| **必改-3（`uk_ppera` 2 线程档过期）** | ✅ **已闭环** | `:909` 与 `:1055` 均已回填「去锁后槽位隔离复验**已于 M5 第三部分完成**（2/3/4 线程三档 + `thread_mode=0` 退化档全部 PASS）」并标【实测】 |
| **必改-4（探针行号/处数）** | ✅ **已闭环** | `:1068`/`:1130` 明确「**全部 6 处** `#if 1 /* M17 temporary probe */`」，并改为以 `grep -rn "M17 temporary probe" lib/` **零命中**为验收判据；摘除后已重跑 clean build 并给出 md5 `df05d2cd078d631ad2d8ee7caba8d387` |
| **必改-5（`ff_glue.c` 跨 commit 拆分）** | ✅ **已闭环，且处理得比我的建议更细** | `:1105`/`:1131` 新增「跨 commit 的同一文件须按 hunk 拆分」，`reviewer` 已确认两个 commit **各自可独立编译**，并说明**实际采用的手法不是 `git add -p`**（见其 §8.3.1）；同时 `:1130` 给出了我未想到的反面理由 —— 对**同一 commit 内的探针**不宜用 `git add -p`，因为会在索引中留下「探针调用了未stage 的辅助函数」这类**不可编译中间态** |
| **必改-6（DoD-8 清单不全）** | ✅ **已闭环** | `:1068` 已补齐我实测列出的四类残留：`example/helloworld_g1_prelock`、`example/helloworld_g2_nolock`、`config.m17_g2_2t.ini`、`config.m17_g2_4t.ini`、`/tmp/m17*`，并明确「一律通过 `/data/workspace/rm_tmp_file.sh` 执行」、探针属源码改动走正向修改；还补了一条我未提的操作优化：两个二进制副本**保留到 M7 最后一步再删**，以便随时复核 DoD-5 的 A/B 数据 |

## 11.2 commit message 规约核验✅

三次提交均为**英文**且在 **1~3 句**内（`c7996a94f` / `57b612d16` / `06396b501`），符合规约；`c7996a94f` 的正文完整交代了「三元组在 `uma_startup1()` 之前设置」「稠密pcpu id + per-thread `curcpu`」「`uma_page` slab hash 提前初始化」三项，可追溯到 §4.3/§4.5/§4.10。

## 11.3 gate-design 结案声明

- 我在M2 共出具**4 轮**门禁结论：第 1 轮 **FAIL**（§5~§9 缺失，10 条必改）→ 第 2 轮 **PASS-with-fixes**（3 条必改）→ 第 3 轮 **PASS-with-fixes** → 第 4 轮（bounce 1 收尾）**PASS**（6 条必改）→ 本节确认 **6 条全部闭环**。**M2 bounce 最终 = 1/3，未触及上限。**
- 我在本项目中共提交 **3 次自我更正**（均因误用「带探针补丁的工作区」行号而非 HEAD 行号：`ck_epoch` 块、`ip_fw_dynamic.c` 在 Makefile 的位置、`arc4random_uniform.c` 的位置；其中第 1 轮的 E5 建议是**错误建议**，`designer` 正确地拒绝了它）。教训已写入本报告 §4 与 §9.5，供后续同类任务参考：**凡核验被改动过的文件行号，一律先`git show HEAD:<file>`。**
- 未落盘材料：**无**。本报告（§0~§11）即gate-design 的完整记录。
- 边界重申：我的全部结论均为**静态审核 + 文件系统只读实测**，**从未做过运行时验证**；所有运行期结论以 `tester` 的 `_m17_F_runtime.md` 为准。
