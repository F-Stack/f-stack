...
# 第六部分：物理机人工校验（2026-08-06）

> 本部分由用户在物理机上人工执行，非 agent tester 自动化测试。记录 native-mt 多线程模式在物理机上的最终人工校验结论。

## V1. 校验范围与结论

| 校验项 | 结论 | 说明 |
|---|---|---|
| **功能验证** | **PASS** | `thread_mode=1` 多线程模式下，业务功能正常（HTTP 服务可访问、连接建立与数据收发正常）。 |
| **性能测试** | **PASS** | wrk 压测吞吐正常，与本文第一~四部分实测数据一致（2 线程 ~236k req/s 量级）。 |

**整体判定**：native-mt SMP-aware pcpu/SMR 槽位隔离 + 全局锁移除（G1+G2）的**功能与性能在物理机上验证通过**，可进入生产试运行阶段（见 §V3 残留风险）。

## V2. 已确认通过的项

- **DoD-1 槽位隔离**：1/2/3/4 线程 + `thread_mode=0` 1/2 进程，per-cpu 槽位隔离实测成立（本文第二/三部分已实测，物理机校验一致）。
- **DoD-2 去锁**：`uma_crit_lock` 已从二进制移除（符号级判据，本文第三部分 X0）。
- **DoD-4 矩阵**：六档零崩溃、零 socket error、日志零新增（本文第二/三/四部分）。
- **DoD-5 去锁前后吞吐**：A/B 交叉复测 PASS（本文第三部分 X4.1）。
- **`thread_mode=0` 零回归**：D7 实测成立（本文第二/三部分 R3）。

## V3. 残留风险（本轮不修，后续优化）

以下残留风险已如实记录，本轮不修复，留待后续专项处理：

| # | 残留风险 | 处置 |
|---|---|---|
| 1 | **ipfw/netisr 的 DPCPU 槽位别名** | 需 DPCPU 独立立项（spec 17 §6.3 + §6.15） |
| 2 | **counter(9) 统计竞争** | 既存，非本轮引入（spec 17 §6.7） |
| 3 | **tcp_hpts 实例数 1→N 的 callout 归属错配（R6）** | CM6/CM7 缩锁时处理（spec 17 §6.19） |
| 4 | **net.isr.dispatch 必须保持 direct** | 配置约束，已文档化（spec 17 §6.18） |
| 5 | **ff_subr_prf.c 全局无锁行缓冲** | 既有日志缺陷，非产品正确性风险（本文第三部分 X1.4 已抓到拼接污染实证） |
| 6 | **ff_pthread_create 线程不支持调用 ff_*** | fail-fast 设计，已文档化（spec 17 §6.5/§6.17） |
| 7 | **wrk 压测下进程偶发 crash（非必现）** | **本轮新增**：需多次压测复现 → 抓崩溃栈 → 定位根因。详见 spec 17 §6.23。 |

## V4. 规约合规声明（物理机校验部分）

- 本部分为用户在物理机上的人工校验记录，结论性陈述，未涉及 `rm`/`kill`/`chmod` 等命令操作。
- 校验使用的 `config.ini` 为本机运行环境值（不入库），与本文前五部分的测试环境一致。
- 所有结论均基于实际物理机运行结果，无臆测。

---

**物理机人工校验完结。native-mt 多线程模式功能与性能验证通过，残留风险已如实记录，偶发 crash 留待后续排查。**
