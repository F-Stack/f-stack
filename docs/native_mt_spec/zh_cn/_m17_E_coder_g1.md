...
2. **`kill_process.sh <name>` 按进程名匹配存在误伤 `tester` 进程的风险**。本次**经审计确认未误伤**：最后一次 kill 的审计快照 `/data/workspace/.trash/20260804-061609-988598-kill/` 内**只有 `kill_987958.snap`**（我自己的 pid，快照时间 `14:16:10`），而 `tester` 的 pid `988684` 启动于 `14:16:14`（晚于枚举时刻），未被纳入，且现仍存活。
