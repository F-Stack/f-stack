...
2. **`kill_process.sh <name>` by-process-name matching risks collateral-killing `tester`'s processes**. This time the audit confirmed no collateral kill: the last kill's audit snapshot `/data/workspace/.trash/20260804-061609-988598-kill/` **only contains `kill_987958.snap`** (my own pid; snapshot time `14:16:10`); `tester`'s pid `988684` started at `14:16:14` (after the enumeration moment) and was not included, and is still alive.
