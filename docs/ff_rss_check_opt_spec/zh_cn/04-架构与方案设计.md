...
- **流量分发略不均（历史 R-B 现象）**：recheck=0 时反算出的 sport 经**网卡实际 RSS** hash 后曾大概率落非本队列（历史单候选等价率 ~22%-27%）。**根因修正（R-F，见 §3-quater）**：此前归因于「字节序不等价」有误——真因是 `rte_thash_add_helper` 改写了 `ctx->hash_key` 而 check/NIC 用原始 key（三方 key 不一致）。R-F 令三方 key 对齐后等价率应达 ~100%，此边界随 R-F 修复而消除；recheck 保留为 debug 复核开关。
- **TCP/UDP 连接正确性不受影响**：
...
## 3-quater. 需求 R-F 方案：thash 三方 key 对齐 + `thash_adjust` 开关（归并 _rf_work/design_rf）

> 归并 `_rf_work/design_rf.md`（leader 设计 + 独立 reviewer 复核 PASS）中**与当前代码一致**的方案。根因见 02 §6-quater。路线③ = ①根因修复为主 + ②软扫描兜底，运行时开关切换。

### 3-quater.1 路线①：v4/v6 串行 key 构造并发布 KEY_FINAL（primary）

`ff_rss_thash_build_key(port_id, reta_size)`（`lib/ff_dpdk_if.c:3027`）在 `dev_configure` **之前**执行（`init_port_start` L758-761，`nb_queues>1 && thash_adjust` 时调用）：

```
原始 rsskey (40B)
  ├─[v4] init_ctx("ff_rss_thash_%u", rsskey) + add_helper("sport", len=16, off=FF_RSS_THASH_V4_SPORT_OFF=80)
  │        → rte_thash_get_key(ctx_v4) = KEY_V4（sport 参与字节段被 v4 LFSR 改写，其余原始）
  ├─ 以 KEY_V4 作为 v6 ctx 的种子 key（串行构造，保证 v6 hash 依赖的 v4 改写段一致）
  ├─[v6] init_ctx("ff_rss_thash6_%u", KEY_V4) + add_helper("sport", len=16, off=FF_RSS_THASH_V6_SPORT_OFF=272)
  │        → rte_thash_get_key(ctx_v6) = KEY_FINAL（v4 段 + v6 段均改写，其余原始）
  └─ 发布：memcpy 到持久 buffer（rte_malloc），全局 rsskey ← KEY_FINAL；由调用方 dev_configure 编程进 NIC
```

正确性（位级）：NIC = KEY_FINAL；v4 check 用 KEY_FINAL 前 16B = ctx_v4 key 前 16B（三方一致）；v6 check 用 KEY_FINAL 前 40B = ctx_v6 key（三方一致）。

> **与早期 design 的差异（以代码为准）**：早期设计设想在 `ff_rss_thash_ctx_init` 内 `rte_eth_dev_rss_hash_update` **post-start** 上传 key；当前代码改为在 **dev_configure 之前**由 `build_key` 发布全局 `rsskey`，让调用方原有的 dev_configure 路径把 KEY_FINAL 编程进 NIC（对 mlx5 等仅此路径可靠），不再使用 post-start `rss_hash_update`。`ff_rss_thash_ctx_init(void)`（L3189，primary）退化为启动后读回 NIC key/RETA 的**诊断**核对。

### 3-quater.2 secondary：find_existing 共享单一 ctx

secondary（`rte_eal_process_type()==RTE_PROC_SECONDARY`）**不 init/不改 NIC**：`rte_thash_find_existing(name)` + `rte_thash_get_helper(ctx,"sport")` 复用 primary 在共享 hugepage 上的 ctx，取回同一 KEY_FINAL 同步本进程全局 `rsskey`，保证本进程 `ff_rss_check` 与 NIC 一致。find_existing 失败（异常）→ 本进程禁用 adjust、走软扫描（残余风险见 SOP P5）。

### 3-quater.3 路线②软扫描兜底 + `thash_adjust` 开关

- 新增 `[rss_check] thash_adjust`（`struct ff_rss_check_cfg.thash_adjust`），`rss_check_cfg_handler` 首次 calloc 后显式置 1（`ff_config.c:946`），`thash_adjust=` 行覆盖（L956-957）；`rss_check_cfgs==NULL` 视为 1。
- **与 `rss_check.enable` 解耦**：`thash_adjust` 门控 `build_key`（`ff_dpdk_if.c:757-760`）、`ctx_init`（L1491-1493）、以及 `ff_rss_adjust_sport[6]` 的 route② 守卫（L3262-3264 / L3701-3703，`thash_adjust=0` 时直接 `return -1` 走内核软扫描）；`ff_rss_tbl_init`/`ff_rss_tbl6_init` 仍归 `enable`。
- 真机若 `rss_hash_update`/改 key 不可用，用户改 `thash_adjust=0` 即切软扫描，无需改码重编。

### 3-quater.4 反算范围约束（`first,last` 新形参）

`ff_rss_adjust_sport[6]` **新增 `uint16_t first, uint16_t last`**（内核临时端口范围，`in_pcb.c` 传入）：把候选端口对齐到 `[first,last]` 内的 reta_size 对齐块（`ff_dpdk_if.c:3297-3308`），tuple 以 **reply（inbound SYN-ACK）字段序**构造（remote/local/dport=80/localPort），解出端口后 defensively 校验落在 `[first,last]`（L3352）。这保证反算出的源端口既落本队列、又是合法临时端口。

### 3-quater.5 多 port 残余约束与诊断门控

- **多 port**：全局 `rsskey` 为单实例指针、`ff_rss_check`/`adjust` 不按 port 区分 key，本期仅对**首个有效 port**（`reta_size>=2`）串行构造+发布，其余 port 走软扫描（与 F-Stack 主流单 port 部署一致）；循环入口保存 `orig_rsskey`，每轮基于 orig 而非已替换的全局指针，防污染/泄漏。
- **rss_hf 回填**（若走 hash_update 路径）：Intel PMD 把 `rss_hf==0` 当作关 RSS，须先 `rss_hash_conf_get` 回填当前 hf。
- **诊断门控**：`ff_rss_diag_dump_key` 等 key/RETA hex-dump 由编译宏 **`FF_RSS_DIAG` 门控，默认关闭，不影响数据面**。
