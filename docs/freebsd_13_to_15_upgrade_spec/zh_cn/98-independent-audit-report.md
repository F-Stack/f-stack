...
**建议**：定义两个不同维度：`风险等级` 与 `任务优先级`。mips 作为"上游事实"可 P2，但作为"必须清理任务"可 P0；需在表头写清楚。
...
| 建议 | 在 06 增加每个 TC 的"最小配置文件路径 + 命令 + 预期输出"，否则 QA 仍需二次设计测试方案 |
...
2. 修正 `if_t` 类型定义，保留"访问方式不透明化"但去掉 `void *` 说法。
3. 重新生成真实 `diff -rq` / checksum 的 `04` 统计，或把现有统计全部降级为估算。
...
1. 统一 P0/P1/P2/P3 口径，区分风险等级与任务优先级。
2. 统一 57/75/18/19/24 等任务数字口径，给一个唯一"实施任务台账"。
