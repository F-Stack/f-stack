...
|---|---|---|---|---|---|---|
| 1 | `netinet/in_pcb.c`（IP_BIND v4） | RSS 端口范围 / lport 检查 / IP_BIND_ADDRESS_NO_PORT 相关 hunk1/hunk2 | **已移植**（commit `ff9e3c449`，R-E）：bind(addr,0) 不抢端口、延迟到 connect 走 RSS 选端口；**setsockopt 层已接线**（commit `a2537e143`）：`lib/ff_syscall_wrapper.c` 拦截 `LINUX_IP_BIND_ADDRESS_NO_PORT(24)` 返回成功 no-op（避免与 FreeBSD `IP_BINDANY(24)` 数值碰撞致 v6 EINVAL / v4 误设 INP_BINDANY） | **高** | bind(addr,0)-then-connect 绕过 RSS 选端口，连接 RSS 落核错配 | **已完成**；详细方案见 `ff_rss_check_opt_spec` **R-E**（§6 + §6.9 setsockopt 层补丁） |
